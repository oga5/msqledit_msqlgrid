...
void CConnectInfoDlg::AddData(CString user, CString passwd, CString dbname, CString host, 
	CString port, CString charset, CString connect_name)
{
	LV_ITEM		item;
	...
	EditData(item.iItem, user, passwd, dbname, host, port, charset, connect_name);
}

void CConnectInfoDlg::EditData(int row, CString user, CString passwd, 
	CString dbname, CString host, CString port, CString charset, CString connect_name)
{
	...
	m_list.SetItemText(row, LIST_CHARSET, charset);
...
void CConnectInfoDlg::OnBtnAdd() 
{
	...
	if(selected_row == -1) {
		AddData(m_edit_user, m_edit_passwd, m_dbname, m_host, m_port, m_charset, m_connect_name);
	} else {
		EditData(selected_row, m_edit_user, m_edit_passwd, m_dbname, m_host, m_port, m_charset, m_connect_name);
	}
}
...
void CConnectInfoDlg::SaveConnectInfo()
{
	...
	for(i = 0; i < m_list.GetItemCount(); i++) {
		...
		charset = m_list.GetItemText(i, LIST_CHARSET);
		connect_name = m_list.GetItemText(i, LIST_CONNECT_NAME);

		connect_list.add_list(user.GetBuffer(0), passwd.GetBuffer(0),
			dbname.GetBuffer(0), host.GetBuffer(0), port.GetBuffer(0), charset.GetBuffer(0),
...
void CConnectInfoDlg::CheckSelect()
{
	...
	if(selected_row == -1) {
		...
		m_charset = m_list.GetItemText(selected_row, LIST_CHARSET);
...
void CConnectInfoDlg::SwapItem(int item1, int item2)
{
	CString		user;
	CString		passwd;
	CString		dbname, host, port, charset, connect_name;
	DWORD_PTR	param;
	...
	charset = m_list.GetItemText(item2, LIST_CHARSET);
	...
	m_list.SetItemText(item2, LIST_CHARSET, m_list.GetItemText(item1, LIST_CHARSET));
	...
	m_list.SetItemText(item1, LIST_CHARSET, charset);
