	...
	CConnectList* connect_list, BOOL reload_list)
{
	...
	for(item_idx = 0, r = 0, pos = p_list->GetHeadPosition(); pos != NULL; r++, p_list->GetNext(pos)) {
		...
		if(hreg != NULL) {
			if(!CheckRegexp(hreg, p_info->m_user.GetBuffer(0)) &&
				...
				!CheckRegexp(hreg, p_info->m_charset.GetBuffer(0)) &&
		...
		list_ctrl.SetItemText(item_idx, LIST_CHARSET, p_info->m_charset);
	...
	list_ctrl.SetColumnWidth(LIST_CHARSET, LVSCW_AUTOSIZE_USEHEADER);
...
void CLoginDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_OPTION, m_charset);
	//}}AFX_DATA_MAP
	DDX_Control(pDX, IDC_EDIT_HOST, m_edit_host);
	DDX_Control(pDX, IDC_EDIT_OPTION, m_edit_charset);
}
...
BOOL CLoginDlg::OnInitDialog() 
{
	...
	if(my_library_is_ok() == FALSE) {
		MessageBox(_T("libmysql.dllが初期化されていません"), _T("Error"), MB_ICONEXCLAMATION | MB_OK);
		OnCancel();
	}
	...
	if(m_port == _T("")) m_port = _T("3306");
	if(m_charset == _T("")) m_charset = _T("utf8mb4");
	UpdateData(FALSE);
	...
	m_list.InsertColumn(LIST_CHARSET, _T("CHARSET"), LVCFMT_LEFT, 100);
...
void CLoginDlg::OnOK() 
{
	...
	m_connect_str.Format(_T("host='%s' port='%s' dbname='%s' user='%s' charset='%s'"),
		m_host, m_port, m_dbname, m_user, m_charset);

	m_ss = my_login(m_host, m_user, m_passwd, m_dbname, m_port, m_charset, msg_buf);
...
int CLoginDlg::GetConnectInfoIdx()
{
	...
	for(i = 0; i < m_list.GetItemCount(); i++) {
		if(m_list.GetItemText(i, LIST_USER) == m_user && 
			...
			m_list.GetItemText(i, LIST_CHARSET) == m_charset) {
			return i;
		}
	}
...
void CLoginDlg::OnItemchangedList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if((p_nmhdr->uNewState & (LVIS_SELECTED | LVNI_ALL)) != 0) {
		...
		m_charset = m_list.GetItemText(p_nmhdr->iItem, LIST_CHARSET);
	} else {
		m_user = _T("");
		m_passwd = _T("");
		m_dbname = _T("");
...
void CLoginDlg::RelayoutControl()
{
	...
	m_edit_charset.GetWindowRect(charset_rect);
	ScreenToClient(charset_rect);
	...
	m_edit_charset.SetWindowPos(NULL, charset_rect.left, charset_rect.top,
		client_rect.Width() - charset_rect.left - 10, charset_rect.Height(), SWP_NOMOVE | SWP_NOZORDER);
