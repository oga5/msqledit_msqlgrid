...
CConnectInfo::CConnectInfo(TCHAR *user, TCHAR *passwd, TCHAR *dbname, TCHAR *host, TCHAR *port, TCHAR *charset,
	TCHAR *connect_name)
{
	m_user = user;
	m_passwd = passwd;
...
BOOL CConnectList::add_list(TCHAR *user, TCHAR *passwd, TCHAR *dbname, TCHAR *host, TCHAR *port, TCHAR *charset,
	TCHAR *connect_name)
{
	CConnectInfo *p_info = new CConnectInfo(user, passwd, dbname, host, port, charset, connect_name);
...
static void make_entry(TCHAR *user_entry, TCHAR *passwd_entry, TCHAR *dbname_entry,
	TCHAR *host_entry, TCHAR *port_entry, TCHAR *charset_entry, TCHAR *connect_name_entry,
	int i)
{
	...
	_stprintf(charset_entry, _T("CHARSET-%d"), i);
...
BOOL CConnectList::load_list()
{
	...
	for(i = 0; i < cnt; i++) {
		make_entry(user_entry, passwd_entry, dbname_entry, host_entry, port_entry, charset_entry,
		...
		charset = AfxGetApp()->GetProfileString(_T("CONNECT_INFO"), charset_entry, _T(""));
		...
		add_list(user.GetBuffer(0), passwd.GetBuffer(0), dbname.GetBuffer(0),
			host.GetBuffer(0), port.GetBuffer(0), charset.GetBuffer(0),
...
BOOL CConnectList::save_list()
{
	...
	for(i = 0, pos = m_connect_list.GetHeadPosition(); pos != NULL; i++, m_connect_list.GetNext(pos)) {
		...
		AfxGetApp()->WriteProfileString(_T("CONNECT_INFO"), charset_entry, p_info->m_charset);
