...
static int sql_run_main(CMsqleditDoc* pdoc, _thr_sql_run_st* sql_run_st, HWND hWnd)
{
	...
	if(!g_option.sql.auto_commit &&
		my_trans_is_idle(g_ss) &&
		!command_no_begin(psql)) {
		ret_v = my_exec_sql(g_ss, _T("BEGIN"), g_msg_buf);
	}
...
static int sql_run(_thr_sql_run_st *sql_run_st)
{
	...
	my_notice_processor org_notice_processor = my_set_notice_processor(g_ss, 
...
//		int		sql_type;
		...
		if(*(sql_run_st->cancel_flg) == 1) {
			ret_v = MYERR_CANCEL;
			_stprintf(g_msg_buf, MYERR_CANCEL_MSG);
		}
		...
		if(g_option.sql_logging) {
			g_sql_logger->Save(my_user(g_ss), my_db(g_ss), my_host(g_ss), my_port(g_ss),
	...
	if(g_option.sql.receive_async_notify) {
		my_notice(g_ss, g_msg_buf);
	}

	my_set_notice_processor(g_ss, org_notice_processor, NULL);
...
void CMsqleditDoc::OnFileSaveGridAs()
{
	...
	if(check_ext(file_name, _T(".txt")) == TRUE) {
		ret_v = my_save_dataset_tsv(file_name, m_dataset,
			g_option.download.put_column_name, g_msg_buf);
	} else {
		ret_v = my_save_dataset_csv(file_name, m_dataset,
			g_option.download.put_column_name, g_msg_buf);
	}
...
void CMsqleditDoc::OnDownload()
{
	...
	if(!uni_ar.OpenFile(file_name, _T("w"), KANJI_CODE_SJIS, LINE_TYPE_CR_LF)) {
		_stprintf(g_msg_buf, MYERR_OPEN_FILE_MSG, file_name);
...
BOOL CMsqleditDoc::CreatePsqlGridFile(TCHAR *sql, const TCHAR *file_name)
{
	...
	HMyDataset		tab_def_dataset = NULL;
	...
	if(tab_def_dataset == NULL || my_dataset_row_cnt(tab_def_dataset) == 0) {
//		char		msg_buf2[1024];
...
//		strcpy(msg_buf, msg_buf2);
	...
	// create column list
	{
		...
		for(;;) {
			...
			if(_tcscmp(buf, _T("*")) == 0) {
				for(int idx = 0; idx < my_dataset_row_cnt(tab_def_dataset); idx++) {
					column_array.Add(my_dataset_data2(tab_def_dataset, idx, _T("ATTNAME")));
TRACE(_T("COLUMN_NAME: %s\n"), my_dataset_data2(tab_def_dataset, idx, _T("ATTNAME")));
				}
			} else {
				...
				int idx;
				for(idx = 0; idx < my_dataset_row_cnt(tab_def_dataset); idx++) {
					if(_tcscmp(col_name, my_dataset_data2(tab_def_dataset, idx, _T("ATTNAME"))) == 0) break;
				}
				if(idx == my_dataset_row_cnt(tab_def_dataset)) {
...
TRACE(_T("COLUMN_NAME: %s\n"), col_name);
...

	{
		...
		if(fp == NULL) {
			...
			if(tab_def_dataset) my_free_dataset(tab_def_dataset);
			return FALSE;
		}
