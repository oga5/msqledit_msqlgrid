...
//		AfxMessageBox(IDP_OLE_INIT_FAILED);
	...
	if(my_init_library(g_msg_buf) != 0) {
	...
	// ログイン
	if(my_library_is_ok() == FALSE || Login() != 0) {
		g_login_flg = FALSE;
		// ステータスバーに，ログイン情報を表示
...
void CPsqleditApp::PrintConnectInfo()
{
	...
	HMyDataset dataset = my_create_dataset(g_ss, _T("select version()"), g_msg_buf);
	...
	message.Format(
		...
		_T("---------------------------------------------------------------------\n"),
		my_dataset_data(dataset, 0, 0),
		my_user(g_ss), my_db(g_ss), my_host(g_ss), my_port(g_ss));
