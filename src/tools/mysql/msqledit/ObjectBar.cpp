...
int CObjectBar::SetObjectList(HMyDataset dataset, const TCHAR *object_name)
{
	...
	int comment_idx = my_dataset_get_col_no(dataset, _T("comment"));
	int regex_result;

	for(r = 0, item_idx = 0; r < my_dataset_row_cnt(dataset); r++) {
		if(hreg != NULL) {
			OREG_DATASRC data_src;
			data_str = my_dataset_data(dataset, r, 0);
			data_str.MakeLower();
...
int CObjectBar::SetUserList(const TCHAR *user_name)
{
	...
	for(i = 0; i < my_dataset_row_cnt(dataset); i++) {
		...
		if(_tcscmp(user_name, my_dataset_data(dataset, i, 0)) == 0) {
...
int CObjectBar::SetListColumns(HMyDataset dataset)
{
	if(dataset == NULL) return 0;
	ASSERT(my_dataset_col_cnt(dataset) < MAX_COLUMN_CNT);
	...
	// カラムを追加
	for(c = 0; c < my_dataset_col_cnt(dataset); c++) {
		if(list_column_width[idx_new][c] < 0) {
			if(my_dataset_get_colsize(dataset, c) > 0 && my_dataset_get_colsize(dataset, c) < 20) {
				col_width = (my_dataset_get_colsize(dataset, c) + 1) * m_font_height / 2;
			} else {
		...
		p_list->InsertColumn(c, my_dataset_get_colname(dataset, c), LVCFMT_LEFT, col_width);
	}
