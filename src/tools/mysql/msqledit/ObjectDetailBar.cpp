...
int CObjectDetailBar::SetObjectList(HMyDataset dataset)
{
	...
	int item_idx;
	int comment_idx = my_dataset_get_col_no(dataset, _T("description"));
	...
	for(r = 0, item_idx = 0; r < my_dataset_row_cnt(dataset); r++) {
		if(m_list_type == COLUMN_LIST && hreg != NULL) {
			...
			CString data_str = my_dataset_data(dataset, r, TABLE_COLUMN_NAME);
			data_str.MakeLower();
			...
			if (regex_result != OREGEXP_FOUND) {
				if (g_option.object_list_filter_column == OFC_FIRST_AND_COMMENT_COLUMN && comment_idx > 0) {
					// コメントを検索
					data_str = my_dataset_data(dataset, r, comment_idx);
					data_str.MakeLower();
					oreg_make_str_datasrc(&data_src, data_str);
					regex_result = oreg_exec2(hreg, &data_src);
				}
				if (g_option.object_list_filter_column == OFC_ALL_COLUMN) {
					for (c = 1; c < my_dataset_col_cnt(dataset); c++) {
						data_str = my_dataset_data(dataset, r, c);
						data_str.MakeLower();
						oreg_make_str_datasrc(&data_src, data_str);
						regex_result = oreg_exec2(hreg, &data_src);
		...
		item.iItem = item_idx;
		item.lParam = r;
		item.pszText = (TCHAR *)my_dataset_data(dataset, r, 0);
		item.iItem = p_list->InsertItem(&item);
		if(item.iItem == -1) goto ERR1;

		for(c = 1; c < my_dataset_col_cnt(dataset); c++) {
			if(m_list_type == COLUMN_LIST && c == DATA_LENGTH &&
				_tcscmp(my_dataset_data(dataset, r, DATA_TYPE), _T("numeric")) == 0) {

				int len = _ttoi(my_dataset_data(dataset, r, DATA_LENGTH));
				...
				p_list->SetItemText(item.iItem, c, my_dataset_data(dataset, r, c));
			}
		}
...
int CObjectDetailBar::SetIndexList()
{
	...
	m_list.DeleteAllItems();
	m_list.SetItemCount(my_dataset_row_cnt(dataset));
	...
	for(i = 0; i < my_dataset_row_cnt(dataset); i++) {
		...
		if(index_name != my_dataset_data(dataset, i, 0)) {
			index_name = my_dataset_data(dataset, i, 0);
			if(m_list.SetItemText(item.iItem, INDEX_NAME,
				my_dataset_data(dataset, i, 0)) == FALSE) goto ERR2;
			if(m_list.SetItemText(item.iItem, INDEX_IS_UNIQUE,
				my_dataset_data(dataset, i, 2)) == FALSE) goto ERR2;
			if(m_list.SetItemText(item.iItem, INDEX_IS_PRIMARY,
				my_dataset_data(dataset, i, 3)) == FALSE) goto ERR2;
			if (m_list.SetItemText(item.iItem, INDEX_OID,
				my_dataset_data(dataset, i, 4)) == FALSE) goto ERR2;
			if (m_list.SetItemText(item.iItem, TABLE_OID,
				my_dataset_data(dataset, i, 5)) == FALSE) goto ERR2;
		}

		if(m_list.SetItemText(item.iItem, INDEX_COLUMN_NAME,
			my_dataset_data(dataset, i, 1)) == FALSE) goto ERR2;
	}
...
int CObjectDetailBar::SetPropertyList()
{
	...
	m_list.DeleteAllItems();
	m_list.SetItemCount(my_dataset_col_cnt(dataset));
	...
	item.mask = LVIF_PARAM;
	item.iSubItem = 0;
	for(i = 0; i < my_dataset_col_cnt(dataset); i++) {
		no.Format(_T("%d"), i + 1);
		item.iItem = i;
		item.lParam = i;
		item.iItem = m_list.InsertItem(&item);
		...
		if(m_list.SetItemText(item.iItem, PROPERTY_NAME,
			my_dataset_get_colname(dataset, i)) == FALSE) goto ERR2;
		if(m_list.SetItemText(item.iItem, PROPERTY_VALUE,
			my_dataset_data(dataset, 0, i)) == FALSE) goto ERR2;
	}
...
CString CObjectDetailBar::MakeColumnListAll(int row_len, CString *insert_values, BOOL b_alias)
{
	...
	for(i = 0; i < my_dataset_row_cnt(dataset); i++) {
		make_object_name(&column_name, my_dataset_data(dataset, i, 0),
			g_option.text_editor.copy_lower_name);
		if(!paste_table_name.IsEmpty()) {
			column_name = paste_table_name + _T(".") + column_name;
		}
		...
		// 最後のカラム以外は，', 'を付ける
		if(i != my_dataset_row_cnt(dataset) - 1) {
			paste_str += ", ";
			row_len += 2;
