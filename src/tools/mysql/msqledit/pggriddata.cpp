...
int CMyGridData::GetMaxColLen(int col, int limit_len)
{
	int		i, len, max_len;
...
//	if(col_max_size < 0) col_max_size = INT_MAX;
	...
	for(i = 0, max_len = 0; i < row_cnt; i++) {
		len = (int)inline_strlen_first_line(my_dataset_data(m_dataset, i, col), 2);
		if(len > max_len) {
			max_len = len;
...
CMyGridData::~CMyGridData()
{
	ClearRowIdx();
}

void CMyGridData::ClearRowIdx()
{
	if(m_row_idx != NULL) {
		free(m_row_idx);
		m_row_idx = NULL;
...
void CMyGridData::SetDataset(HMyDataset dataset) 
{
	ClearRowIdx();
	m_dataset = dataset;
	AllocDispInfo();
...
void CMyGridData::SwapRow(int r1, int r2)
{
