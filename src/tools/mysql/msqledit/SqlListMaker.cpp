...
CSqlListMakerCache_Object::CSqlListMakerCache_Object()
{
	...
	m_dataset_arr = (HMyDataset **)malloc(sizeof(HMyDataset *) * SQL_LIST_MAKER_OBJECT_CACHE_SIZE);
	m_dataset_buf = (HMyDataset *)malloc(sizeof(HMyDataset) * SQL_LIST_MAKER_OBJECT_CACHE_SIZE);
...
CSqlListMakerCache_Object::~CSqlListMakerCache_Object()
{
	...
	for(int i = 0; i < SQL_LIST_MAKER_OBJECT_CACHE_SIZE; i++) {
		my_free_dataset(m_dataset_buf[i]);
		m_dataset_buf[i] = NULL;
	}
	free(m_dataset_arr);
	free(m_dataset_buf);
...
BOOL CSqlListMakerCache_Object::MakeList(CString object_name, HMyDataset *&dataset, BOOL sort_column_name)
{
	CString owner = "";
	CString obj_name = "";
	...
	HMyDataset *dataset_tmp = m_dataset_arr[0];

	my_free_dataset(*dataset_tmp);
	*dataset_tmp = NULL;
	...
	*dataset_tmp = get_column_list_for_list_maker(g_ss, owner.GetBuffer(0), obj_name.GetBuffer(0), sort_column_name, NULL);
	...
	if(my_dataset_row_cnt(*dataset_tmp) == 0) {
		my_free_dataset(*dataset_tmp);
		*dataset_tmp = NULL;
...
BOOL CSqlListMaker::MakeObjectList(CCodeAssistData *assist_data, const CString &object_name)
{
	...
	assist_data->InitData(FALSE);
	for(int i = 0; i < my_dataset_row_cnt(*dataset); i++) {
		assist_data->AddRow();
		assist_data->SetData(i, 0, my_dataset_data(*dataset, i, 0));
		assist_data->SetData(i, 1, my_dataset_data(*dataset, i, 1));
	}
