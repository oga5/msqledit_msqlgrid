...
const static TCHAR *plsql_object_type_list[] = {_T("FUNCTION"), _T("PROCEDURE"), _T("TRIGGER")};
...
/*------------------------------------------------------------------------------
 オブジェクト名リストの取得
------------------------------------------------------------------------------*/
static HMyDataset get_object_name_list(HMySession ss, const TCHAR *owner, 
	CString object_type_list, TCHAR *msg_buf)
{
	TCHAR	sql_buf[4096];

	_stprintf(sql_buf,
		_T("SELECT table_name as relname, table_name as org_relname, \n")
		_T("	CASE table_type WHEN 'BASE TABLE' THEN 'table' WHEN 'VIEW' THEN 'view' ELSE 'other' END as relkind \n")
		_T("FROM information_schema.tables \n")
		_T("WHERE table_schema = '%s'"),
		owner);

	if(object_type_list != _T("")) {
		_tcscat(sql_buf, _T(" AND table_type IN ("));
...
/*------------------------------------------------------------------------------
 フィールドリストの取得(全部)
------------------------------------------------------------------------------*/
static HMyDataset get_all_column_list(HMySession ss, const TCHAR *owner, TCHAR *msg_buf)
{
	TCHAR	sql_buf[4096];

	_stprintf(sql_buf,
		_T("SELECT DISTINCT column_name as attname, column_name as org_attname, \n")
		_T("	concat(data_type, '(', IFNULL(character_maximum_length, numeric_precision), ')') as typename \n")
...
int CSQLStrToken::addDatasetToKeywordList(HMyDataset dataset, const TCHAR *owner, TCHAR *msg_buf,
	int column_idx, int org_column_idx, int type_idx)
{
	int		i, idx, current_word_cnt;

	current_word_cnt = m_completion_word_cnt;
	m_completion_word_cnt = m_completion_word_cnt + my_dataset_row_cnt(dataset);
	...
	for(i = 0, idx = current_word_cnt; i < my_dataset_row_cnt(dataset); i++, idx++) {
		m_completion_words[idx].lwr_str = (TCHAR *)my_dataset_data(dataset, i, column_idx);
		m_completion_words[idx].org_str = my_dataset_data(dataset, i, org_column_idx);
		m_completion_words[idx].str = m_completion_words[idx].lwr_str;
		m_completion_words[idx].type_name = my_dataset_data(dataset, i, type_idx);
...
{
	my_free_dataset(m_dset_table);
	my_free_dataset(m_dset_column);
	m_dset_table = NULL;
	m_dset_column = NULL;
}

int CSQLStrToken::initCompletionWord(HMySession ss,
