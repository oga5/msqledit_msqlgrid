...
static int query_cancel(HWND hWnd, TCHAR *msg_buf)
{
	...
	if(SendMessage(hWnd, CND_WM_QUERY_CANCEL, 0, 0) == 1) {
		_stprintf(msg_buf, MYERR_CANCEL_MSG);
		return MYERR_CANCEL;
	}
	return 0;
}
	...
	const TCHAR *table_name, const TCHAR *sql, BOOL b_data_lock)
{
	...
	m_reserve_columns = my_dataset_row_cnt(m_pkey_dataset);

	if(AllocDataMemory(my_dataset_row_cnt(dataset), my_dataset_col_cnt(dataset)) != 0) return 1;
...
ERR1:
	int ret_v2 = my_rollback(ss, NULL);
	if(ret_v2 != 0) {
		// 致命的なエラー
	...
	if(m_b_data_lock) {
		// データをロックするために，再検索する
		HMyDataset dset_dummy = my_create_dataset(m_ss, m_sql.GetBuffer(0), NULL);
		if(dset_dummy == 0) {
			// 致命的なエラー
...
CString CEditableMyGridData::GetUpdateWhereClause(int row)
{
	...
	where_clause = _T("\nwhere ");
	col_offset = my_dataset_col_cnt(m_dataset) - m_reserve_columns;
	...
	HWND hWnd, volatile int *cancel_flg)
{
	...
	for(row = 0; row < Get_RowCnt(); row++) {
		...
		if(IsDeleteRow(row)) {
			...
			ret_v = my_exec_sql(ss, sql.GetBuffer(0), msg_buf);
			if(ret_v != 0) goto ERR1;
			(*row_cnt)++;
		}
	}
...
BOOL CEditableMyGridData::IsBlank(int row, int col)
{
	...
	if(my_dataset_is_null(m_dataset, GetRowIdx(row), col)) return FALSE;

	const TCHAR *str = my_dataset_data(m_dataset, GetRowIdx(row), col);
