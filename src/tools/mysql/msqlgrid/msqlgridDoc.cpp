...
void CPsqlgridDoc::FreeDataset()
{
	...
	if(m_dataset != NULL) my_free_dataset(m_dataset);
	if(m_pkey_dataset != NULL) my_free_dataset(m_pkey_dataset);
...
int CPsqlgridDoc::SelectTable(BOOL auto_start, const TCHAR *file_name)
{
	...
	if(m_owner.IsEmpty()) m_owner = my_user(g_ss);
...
int CPsqlgridDoc::SetOciDataset(HMyDataset dataset, HMyDataset pkey_dataset, const TCHAR *owner, 
...
void CPsqlgridDoc::OnFileSaveGridAs() 
{
	...
	ret_v = my_save_dataset_ex(file_name, m_dataset, 
...
void CPsqlgridDoc::CalcGridSelectedData()
{
	...
	if(m_dataset == NULL || my_dataset_row_cnt(m_dataset) == 0 || 
